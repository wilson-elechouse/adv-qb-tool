...
const initialClientRef = String(input.client_ref || input.payment_no || `bill-payment-${Date.now()}`);
const memo = tagMemo(input.memo || '');
const payload = normalizePayload(input);

const api = await createSession(base, cfg);

const create = await createDraftWithRetry(api, 'bill_payment', initialClientRef, memo);
